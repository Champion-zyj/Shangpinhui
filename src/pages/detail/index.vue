    ...
    <section class="con">
      ...
      <div class="mainCon">
        ...
        <div class="previewWrap">
          <!--放大镜效果-->
          <Zoom :skuImages="skuInfo"/>
          <!-- 小图列表 -->
          <ImageList :skuImages="skuInfo"/>
        </div>
        <!-- 右侧选择区域布局 -->
        <div class="InfoWrap">
          ...
          <div class="choose">
            <div class="chooseArea">
              ...
              <dl v-for="item in spuSaleAttrList && spuSaleAttrList" :key="item.id">
                <dt class="title">{{item.saleAttrName}}</dt>
                <dd changepirce="0" :class="{active: item2.isChecked == '1'}" 
                v-for="item2 in item.spuSaleAttrValueList" :key="item2.id"
...
<script>
import { reqAddOrUpdateShopCart, reqGetdetailInfo } from '@/api/detail';
...
import { Message } from 'element-ui';
  ...
  export default {
    ...
    data(){
      return {
        categoryView:{},//顶部三级显示导航
        skuInfo:{}, // 右侧商品信息
        spuSaleAttrList:[], //右侧选择参数
        skuNum:1, //购买产品的数量
      }
    },
    computed:{
    ...
    methods:{
      async getDetailInfo(){
        ...
        const {data} = await reqGetdetailInfo(this.$route.params.skuid)
        this.categoryView = data.categoryView 
        this.skuInfo = data.skuInfo
        this.spuSaleAttrList = data.spuSaleAttrList
        console.log(data);
      },
      changeActive(item2,arr){
        arr.forEach(t=>{
          t.isChecked = 0
        })
        item2.isChecked = 1
      ...
      async addShopcar(){
        try {
         const res =  await reqAddOrUpdateShopCart({skuid:this.$route.params.skuid,skuNum:this.skuNum})
         if(res.code === 200){
